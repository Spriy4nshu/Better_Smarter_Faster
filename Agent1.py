...
def agent1(ndict, clist):
    ...
    prey_moves = [14, 13, 12, 7, 6, 11, 10, 15, 14, 13, 12, 11, 12]
    steps = 0
    ...
    while steps < 5000:
        ...
        # preys move
        if len(prey_moves) != 0:
            prey_pos = prey_moves[0]
            prey_moves.remove(prey_moves[0])
        else: 
...
for y in range(3000):
    steps, win_condition = agent1(ndict, clist)
...
print(no_of_wins)
print('done')
data.to_csv('AgentVpartail.csv', index=False)
