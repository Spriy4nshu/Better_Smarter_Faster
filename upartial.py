...
def agent1(ustar, clist):
    ...
    while agent_pos != prey_pos or agent_pos != predator_pos:
        ...
        # agent_pos = min(pos_states, key=pos_states.get)
        steps += 1
...
for y in range(3000):
    steps, win_condition = agent1(ustar, clist)
...
print(wins)
print('done')
data.to_csv('AgentUpartial.csv', index=False)
